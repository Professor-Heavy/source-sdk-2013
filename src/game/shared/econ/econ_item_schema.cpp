...
#define daylight _daylight
	...
	m_bModItem = m_pKVItem->GetInt("mod_tf_item", 0) != 0;
...
bool CEconItemSchema::BInitTextBuffer( CUtlBuffer &buffer, CUtlVector<CUtlString> *pVecErrors /* = NULL */ )
{
	...
	if ( m_pKVRawDefinition->LoadFromFile(g_pFullFileSystem, "scripts/items/restored_items_game.txt", "GAME"))	// IMPORTANT: make sure the path matches the name of your item schema!
	{
		return BInitSchema( m_pKVRawDefinition, pVecErrors )
			&& BPostSchemaInit( pVecErrors );
	}
